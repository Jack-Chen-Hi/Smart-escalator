...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagstop_start)
    {
        info->event[info->num] = EVENT_stop_start;
        info->num++;
        dataPoints->valuestop_start = gizStandardDecompressionValue(stop_start_BYTEOFFSET,stop_start_BITOFFSET,stop_start_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuestop_start != cur->valuestop_start)
    {
        GIZWITS_LOG("valuestop_start Changed\n");
        ret = 1;
    }
    if(last->valuemotor_status != cur->valuemotor_status)
    {
        GIZWITS_LOG("valuemotor_status Changed\n");
        ret = 1;
    }
    if(last->valuefault != cur->valuefault)
    {
        GIZWITS_LOG("valuefault Changed\n");
        ret = 1;
    }

    if(last->valuepeople_sum != cur->valuepeople_sum)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuepeople_sum Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    if(dataPoints->valuestop_start >= stop_start_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuestop_start Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(stop_start_BYTEOFFSET,stop_start_BITOFFSET,stop_start_LEN,(uint8_t *)devStatusPtr,dataPoints->valuestop_start);  
    }
    if(dataPoints->valuemotor_status >= motor_status_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuemotor_status Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(motor_status_BYTEOFFSET,motor_status_BITOFFSET,motor_status_LEN,(uint8_t *)devStatusPtr,dataPoints->valuemotor_status);  
    }
    if(dataPoints->valuefault >= fault_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuefault Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(fault_BYTEOFFSET,fault_BITOFFSET,fault_LEN,(uint8_t *)devStatusPtr,dataPoints->valuefault);  
    }
    ...
    devStatusPtr->valuepeople_sum = exchangeBytes(gizY2X(people_sum_RATIO,  people_sum_ADDITION, dataPoints->valuepeople_sum)); 
